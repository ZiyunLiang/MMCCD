...
class GaussianDiffusion:
    ...
    def ddim_sample_loop(
        ...
        return final[final_output], [final]
        # return final[-1][final_output] if progress else final[final_output]
    ...
    def ddim_sample_loop_progressive(
        ...
        indices = list(range(self.num_timesteps))[::-1] # 0 -> T
        ...
        if model_name == 'diffusion':
            ...
            for num_repeat in range(128):
                ...
                for i_forward in indices:
                    t = th.tensor([i_forward] * shape[0], device=device)
                    with th.no_grad():
                        noise1 = th.randn(shape, device=device)
                ...
                for i_backward in indices_2:
                    t_backward = th.tensor([i_backward] * shape[0], device=device)
                    with th.no_grad():
                        # x0_pred = model_backward(cond_backward, **model_kwargs)
